...
RegisterNetEvent("adminmenu:tpmz")
AddEventHandler("adminmenu:tpmz", function(target, coords)
    local source = source
    local rank = players[source].rank
    if not canUse("tpmz", rank) then
        DropPlayer(source, "Permission invalide")
        return
    end
    TriggerClientEvent("adminmenu:setCoords", target, vector3(-75.22, -819.00, 326.17))
    TriggerClientEvent("esx:showNotification", source, "~g~Téléportation effectuée")
end)
